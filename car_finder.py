...
class Vehicle():
    ...
    def is_inside_box(self, box):
        ...
        car_mask = np.zeros(shape=(720,1280))
        box_mask = np.zeros(shape=(720,1280))

        box_mask[box[0][1]:box[1][1], box[0][0]:box[1][0]] = True
        cb = self.bounding_box
        car_mask[cb[0][1]:cb[1][1], cb[0][0]:cb[1][0]] = True

        return (np.any(np.logical_and(box_mask, car_mask)))
...
class VehicleIdentifier():
    ...
    def figure_out_cars(self, windows):
        ...
        # create a set that will hold all cars we saw in this frame
        cars_seen_this_frame = set()
        ...
        for car_box in car_boxes:
            cars_in_box = self.get_car_for_bbox(car_box)
            
            if len(cars_in_box) == 0:
                # 2 things can happen now, due to the bad estimation of cars, maybe this box
                # belongs to one of the cars but that car's bounding box is so much off
                # that it says it does not belong to it
                # So lets keep a set of all boxes that had no car and then match it with the set of 
                # cars that say they have no bounding boxes

                print('SVC/Label says there is a box: {}, lets look for an already maybe_car we might have'.format(car_box))
                ...
                maybe_cars_in_box = self.get_car_for_bbox(car_box, which_cars = self.maybe_cars)

                if len(maybe_cars_in_box) > 0:
                    cars_moved = self.update_maybe_cars(maybe_cars_in_box, car_box)
                    cars_seen_this_frame |= cars_moved
                else:
                    car = self.add_car(car_box)
                    cars_seen_this_frame.add(car)
                    cars_in_box.add(car)
            else:
                cars_seen_this_frame |= set(cars_in_box)

            self.update_car_estimates(cars_in_box, car_box)
            
        self.check_cars_gone_off(cars_seen_this_frame, windows)
    ...
    def check_cars_gone_off(self, cars_seen_this_frame, windows):
        missing_cars = self.cars - cars_seen_this_frame

        for car in missing_cars:
            print('Did not see car #: {} estimated speed {}'.format(car.car_no,car.speed))
            if not self.ok_to_remove_gone_off(car):
                self.cars.add(car)
            
            # Maybe a False Negative
            # Our heatmap thresholded this car, but there was an actual detection?
            # if yes, then we need to update this based on our detection
            ...
            #     nonzeroy, nonzerox = heatmap.nonzero()
            #     bbox = ((np.min(nonzerox), np.min(nonzeroy)), (np.max(nonzerox), np.max(nonzeroy)))

            #     car.update_box(bbox)
            #     cars_seen_this_frame[car] = True
            # else:
            #     self.car_gone_out(car)

    def add_car(self, car_box):
        car_no = len(self.cars) + 1
        ...
        if (len(self.cars) > 0) and (not self.very_close_to_other(car_box)):
            self.cars.add(car)
        else:
            self.maybe_cars.add(car)

        return car

    # returns cars, which have been moved from maybe to actual cars
    def update_maybe_cars(self, cars_in_box, car_box):
        cars_moved = set()
        print('Ok we found {} maybe cars, lets see if this one was recent or very old'.format(len(cars_in_box)))

        for car in cars_in_box:
    ...
    def update_car_estimates(self, cars_found, car_box):
    ...
    def ok_to_remove_gone_off(self, car):
        car.miss_frames += 1
