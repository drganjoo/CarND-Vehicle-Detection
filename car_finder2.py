...
# will switch it back into class after done with debugging persistance written file
maybe_cars = []
    ...
    def figure_out_cars(self, boxes):
        ...
        for car_box in car_boxes:
            ...
            if car is None:
                ...
                # check in case we already have this bounding box in a maybe car
                # increment frame connt and see if it is to be moved to a car now
                car = self.get_car_for_bbox(car_box, which_cars = maybe_cars)

                if car is not None:
                    print('Maybe Car already found for this frame, checking if it can be moved to active list. Frame count:', car.frame_count)
